...
- ~~Auto-created accounts have an unusable synthetic email + random unrecoverable password — there's no path today to convert one into a real, loggable-in account.~~ **Fixed 2026-08-19**: `auth.ts`'s Google-login handler now checks for exactly one placeholder account (`email LIKE '%@placeholder.internal'`) in the org whose `name` matches the Google profile's first name, and promotes it in place (real email/name/password swapped into the same row, same id) instead of creating a second account — so the person's existing task assignments, notifications, and leave balances become correctly theirs with no manual reassignment. Ambiguous matches (0 or 2+ placeholders with that first name) fall back to a normal new-account signup rather than guessing wrong. Only covers Google login — the plain email/password `/signup` flow has no equivalent check, so signing up that way still creates a separate account from any matching placeholder.
- Auto-created placeholder accounts are hidden from every assignee/participant/owner picker (`GET /users`, `users.ts`, filtered on the same `@placeholder.internal` pattern) — they still exist as real rows with real task history, just invisible in dropdowns until their real owner logs in and gets promoted per the fix above. No visual "placeholder" badge exists anywhere else they might surface (e.g. a task's assignee name on `TasksPage.tsx` itself, outside the picker), which is a narrower remaining gap.
