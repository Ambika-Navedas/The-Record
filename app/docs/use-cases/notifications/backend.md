...
| Method | Path | Notes |
|---|---|---|
| `GET` | `/api/notifications` | Caller's own **unread** notifications only (`read_at IS NULL`), `created_at DESC`, capped at 50 (demo-scale, no pagination). Changed from "all, newest first" to unread-only after user feedback that the panel should empty out once seen, not accumulate a permanent history — since opening the bell (`AppLayout.tsx`) marks everything read right after this fetch, the practical effect is items stay visible for the view that surfaced them, then are gone next time the panel opens. Returns `{ items, unreadCount }` — `unreadCount` is a separate `COUNT(*) WHERE read_at IS NULL` query, not derived from the capped `items` list, so it stays correct even if more than 50 unread notifications exist. Also calls `checkDueReminders(orgId, userId)` first (see `reminders/backend.md`) — this is what makes a due reminder show up as a real notification, since the bell fetches this on every page load. |
...
This gap narrows automatically once someone's placeholder account is promoted (see `auth.ts`'s Google-login placeholder-promotion logic, added 2026-08-19 — same id, so every notification already sitting on that placeholder becomes visible to them the moment they log in for real) — future task assignments on the same (now-real) account start notifying normally through the two producers above.
