...
- `handleToggleNotifications()` — toggles `notifOpen`; on **open** only, refetches `GET /notifications` so anything created since mount shows up.
- Marking read moved from open to **close** (2026-08-20, user feedback: marking read on open cleared the panel while someone was still looking at it) — a separate `useEffect` keyed on `notifOpen`'s true→false transition (tracked via a `wasNotifOpen` ref, since the transition itself is what matters, not either close path individually) fires `POST /notifications/read-all` and optimistically zeroes the badge locally. Keyed off the transition rather than living inside `handleToggleNotifications` because the outside-click effect below sets `notifOpen` directly and never calls that handler — closing by clicking away has to mark read too, not just closing by clicking the bell again.

## Panel
A right-aligned dropdown (`absolute right-0 top-[46px]`, same z-index/shadow convention as the profile menu), fixed-width (`w-80`), scrollable body capped at `max-h-80` for a long list. Each row shows the notification's `message` verbatim (no truncation, no title/body split — messages are short, pre-written sentences from the backend) and a formatted timestamp (`notificationTime()`, a page-local helper combining date + time, since "Aug 17, 2:42 PM" is more useful here than a bare date — unlike most other timestamp displays in this app, a notification's exact time of day matters). No per-item read/unread visual distinction inside the panel (no bold vs. normal text) — since `GET /notifications` now only ever returns unread items (see `notifications/backend.md`'s `GET` row), everything the panel renders is unread by definition, so a read/unread split inside the list would have nothing to distinguish. Closing the panel marks everything it showed as read, so those exact items are gone the next time it's fetched — the panel is a "what's new since you last looked" view, not a permanent history.
